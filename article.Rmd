...
```{r fig2, echo=FALSE, out.width='30%', fig.cap='This figure is an example of a figure caption taking more than one line and justified considering margins mentioned in Section 5.'}
knitr::include_graphics("assets/fig2.jpg")
```

In tables, try to avoid the use of colored or shaded backgrounds, and avoid thick, doubled, or unnecessary framing lines. When reporting empirical data, do not use more decimal digits than warranted by their precision and reproducibility. Table caption must be placed before the table (see Table \ref{tab:tab:variables}) and the font used must also be Helvetica, 10 point, boldface, with 6 points of space before and after each caption.
...
knitr::kable(
  data.frame(
    Variable = c("Distance", "Accuracy", "Time"),
    Relevance = c("High", "High", "Medium"),
    Measure = c("Numeric", "Numeric", "Numeric")
  ),
  format = "latex",
  booktabs = TRUE,
